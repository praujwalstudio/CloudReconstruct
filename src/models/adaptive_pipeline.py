...
class AdaptiveCloudRemoval(nn.Module):
    ...
    def forward(
        ...
        temporal_refs: list[torch.Tensor] = None,
        dem: torch.Tensor = None,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        density = self.density_net(liss4, dem=dem) if dem is not None else self.density_net(liss4)

        # Thin cloud physical correction
        ...
        if sar is not None:
            dense_out = self.sar_fusion(liss4, sar, dem=dem) if dem is not None else self.sar_fusion(liss4, sar)
        ...
        return torch.clamp(output, 0.0, 1.0), density, weights
    ...
    def _compute_blend_weights(self, density: torch.Tensor) -> torch.Tensor:
        ...
        medium = (
            (density[:, 0] >= self.thin_threshold) &
            (density[:, 0] < self.dense_threshold)
        ).float()
...
def build_adaptive_pipeline(
    ...
    density_net = CloudDensityNet(in_channels=3, out_channels=1, dem_channels=dem_channels)
    correction_net = ThinCloudCorrection(in_channels=3)
    temporal_fusion = TemporalFusion(in_channels=3)
    sar_fusion = SARConditionalUNet(sar_channels=2, liss4_channels=3, out_channels=3, dem_channels=dem_channels)
